...
def main():
    ...
    print(f"   Topic: {TOPIC_ID if TOPIC_ID else 'Haupt-Channel'}")
    
    state = load_state()
    first_run = len(state.get('posted_events', set())) == 0
    
    print(f"\n📂 State: {len(state['known_repos'])} repos, {len(state['posted_events'])} events")
    ...
    if not check_rate_limit():
        ...
        print(f"\n📊 Gesamt bekannt: {len(state['known_repos'])}")
    ...
    if unposted_updates:
        print("\n" + "=" * 70)
        print(f"PHASE 3: TELEGRAM POSTING")
        print("=" * 70 + "\n")
